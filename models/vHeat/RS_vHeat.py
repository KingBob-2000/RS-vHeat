...
class Heat2D(nn.Module):
    ...
    def __init__(self, infer_mode=False, res=14, dim=96, hidden_dim=96, capacity_past=None, memory_decay=0.9999, **kwargs):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, hidden_dim, kernel_size=3, padding=1, groups=hidden_dim)
        self.capacity_past = capacity_past
        self.infer_mode = infer_mode
        # if self.capacity_past is not None:
            # self.dwconv_past = nn.Sequential(
            #     nn.Conv2d(dim * 2, dim, kernel_size=3, padding=1, groups=hidden_dim),
            #     LayerNorm2d(dim)
            # )
        self.res = res
        ...
        # self.to_capacity = nn.Sequential(
        #     nn.Linear(hidden_dim, hidden_dim, bias=True),
        #     # nn.Softmax(dim=-1),
        #     # nn.Sigmoid(),
        #     nn.ReLU(),
        ...
        # self.time_scale = nn.Parameter(torch.tensor(0.5), requires_grad=True)

    # hhy0820 (self.freq_embed[i], self.heat_capacity[i], None )
    ...
    def forward(self, x: torch.Tensor, freq_embed=None, heat_capacity=None, heat_capacity_past=None):
        B, C, H, W = x.shape
        # heat_capacity [H W C]
        # heat_capacity [H W C/2]

        if self.capacity_past:
            x_sp = x + heat_capacity.unsqueeze(0).permute(0, 3, 1, 2)
            x_sp_past = x + heat_capacity_past
            x_sp = x_sp + x_sp_past
        else:
            x_sp = x + heat_capacity.unsqueeze(0).permute(0, 3, 1, 2)  # 1hwc
        ...
        x = F.conv1d(x.contiguous().view(-1, M, C), weight_cosm.t().contiguous().view(W, M, 1)).contiguous().view(B, H,
                                                                                                                  W, -1)

        x = self.out_norm(x)

        x = x * nn.functional.silu(z)
        # x = x * nn.functional.silu(z) + x*nn.functional.silu(x_sp)
        x = self.out_linear(x)
...
class HeatBlock(nn.Module):
    def __init__(
        ...
        self.op = Heat2D(infer_mode=infer_mode,res=res, dim=hidden_dim, hidden_dim=hidden_dim, state_dim=3 * hidden_dim, capacity_past=capacity_past, memory_decay=memory_decay)
    ...
    def _forward(self, x: torch.Tensor, freq_embed, heat_capacity, heat_capacity_past):
        if not self.layer_scale:
            if self.post_norm:
                x = x + self.drop_path(self.norm1(self.op(x, freq_embed, heat_capacity, heat_capacity_past)))
                if self.mlp_branch:
                    x = x + self.drop_path(self.norm2(self.mlp(x)))  # FFN
            else:
                x = x + self.drop_path(self.op(self.norm1(x), freq_embed, heat_capacity, heat_capacity_past))
                if self.mlp_branch:
                    x = x + self.drop_path(self.mlp(self.norm2(x)))  # FFN
            return x
        if self.post_norm:
            x = x + self.drop_path(self.gamma1[:, None, None] * self.norm1(self.op(x, freq_embed, heat_capacity, heat_capacity_past)))
            if self.mlp_branch:
                x = x + self.drop_path(self.gamma2[:, None, None] * self.norm2(self.mlp(x)))  # FFN
        else:
            x = x + self.drop_path(self.gamma1[:, None, None] * self.op(self.norm1(x), freq_embed, heat_capacity, heat_capacity_past))
            if self.mlp_branch:
                x = x + self.drop_path(self.gamma2[:, None, None] * self.mlp(self.norm2(x)))  # FFN
        return x

    def forward(self, input: torch.Tensor, freq_embed=None, heat_capacity=None, heat_capacity_past=None):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input, freq_embed, heat_capacity, heat_capacity_past)
        else:
            return self._forward(input, freq_embed, heat_capacity, heat_capacity_past)
...
class RS_vHeat(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2],
                 ...
                 layer_scale=None, use_checkpoint=False, mlp_ratio=4.0, cls_scale=1.5, pretrained=None, img_size=224,mode="opt",
        ...
        self.depths = depths
        if mode =="opt":
            self.patch_embed_opt = StemLayer(in_chans=3,
                                             out_chans=self.embed_dim,
                                             act_layer='GELU',
                                             norm_layer='LN')
        else:
            self.patch_embed_sar = StemLayer(in_chans=1,
                                             out_chans=self.embed_dim,
                                             act_layer='GELU',
                                             norm_layer='LN')
        ...
        #             nn.Conv2d(in_chans, self.embed_dim, kernel_size=patch_size, stride=patch_size),
        #             (LayerNorm2d(self.embed_dim) if patch_norm else nn.Identity()),
        #         )

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule

        self.freq_embed = nn.ParameterList()
        self.heat_capacity = nn.ParameterList()
        for i in range(self.num_layers):
            self.freq_embed.append(
                nn.Parameter(torch.zeros(self.res[i], self.res[i], self.dims[i]), requires_grad=True))
            trunc_normal_(self.freq_embed[i], std=.02)
            self.heat_capacity.append(
                nn.Parameter(torch.zeros(self.res[i], self.res[i], self.dims[i]), requires_grad=True))
                # nn.Parameter(torch.zeros(self.res[i], self.res[i], self.dims[i]), requires_grad=True))
            trunc_normal_(self.heat_capacity[i], std=.02)
        ...
        # self.load_pretrained(ckpt="ckpt_path")
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            for k, v in _ckpt[key].items():
                if "fuse_frequency" in k:
                    continue
                if 'encoder.' in k:
                    new_k = k.replace("encoder.", "")
                    if "freq_embed" in new_k:
                        own_state = self.state_dict()
                        tmp = v.unsqueeze(0)  # [b,h,w,c]
                        tmp = tmp.permute(0, 3, 1, 2)  # [b,c,h,w]
                        tmp = F.interpolate(tmp, size=own_state[new_k].shape[:2],
                                            mode='bilinear', align_corners=False).squeeze(0)  # [c,h,w]
                        v = tmp.permute(1, 2, 0)
                    if "heat_capacity" in new_k:
                        own_state = self.state_dict()
                        tmp = v.unsqueeze(0)  # [b,h,w,c]
                        tmp = tmp.permute(0, 3, 1, 2)  # [b,c,h,w]
                        tmp = F.interpolate(tmp, size=own_state[new_k].shape[:2],
    ...
    def forward_features(self, x, mode="opt"):
        if mode == "opt":
            x = self.patch_embed_opt(x).flatten(2).transpose(1, 2)
        else:
            x = self.patch_embed_sar(x).flatten(2).transpose(1, 2)
        B, L, C = x.shape
        ...
        if self.infer_mode:
            ...
            for i, layer in enumerate(self.layers):
                if i > 0:
                    o_x, x = layer(x, self.freq_embed[i], self.heat_capacity[i],
                                   self.heat_capacity[i - 1])
                else:
                    # o_low, x_img_low = layer(x_img_low, self.freq_embed[i], self.heat_capacity[i], None)
                    o_x, x = layer(x, self.freq_embed[i], self.heat_capacity[i], None)
                    # o_high, x_img_high = layer(x_img_high, self.freq_embed[i], self.heat_capacity[i], None)
        # x = self.fuse_frequency[i](o_low,o_high)
        # x = self.fuse_frequency(o_low, o_high)
        # hhy
        # x = self.fuse_frequency(o_low, x)
        return x
